...
// verify product
$chk=$pdo->prepare("SELECT id FROM products WHERE id=:id"); $chk->execute([':id'=>$product_id]);
...
try {
  ...
  if ($slugs) {
    ...
    if ($slugs) {
      ...
      $sel=$pdo->prepare("SELECT id, slug FROM tags WHERE slug IN ($in)"); $sel->execute($slugs);
      $found=$sel->fetchAll(PDO::FETCH_KEY_PAIR); // slug=>id
      foreach ($slugs as $s) {
        if (!isset($found[$s])) {
          if (!$auto_create) continue;
          $name=ucwords(str_replace('-',' ',$s));
          try { $ins=$pdo->prepare("INSERT INTO tags (name, slug) VALUES (:n,:s)"); $ins->execute([':n'=>$name, ':s'=>$s]); $found[$s]=(int)$pdo->lastInsertId(); }
          catch(PDOException $e){ if($e->getCode()==='23000'){ $r=$pdo->prepare("SELECT id FROM tags WHERE slug=:s"); $r->execute([':s'=>$s]); $found[$s]=(int)$r->fetchColumn(); } else { throw $e; } }
        }
      }
  ...
  if ($ids) {
    $ins=$pdo->prepare("INSERT INTO product_tags (product_id, tag_id) VALUES (:pid,:tid)");
    foreach ($ids as $tid) {
      // verify tag exists
      $tchk=$pdo->prepare("SELECT id FROM tags WHERE id=:id"); $tchk->execute([':id'=>$tid]); if(!$tchk->fetch()) continue;
      $ins->execute([':pid'=>$product_id, ':tid'=>$tid]);
    }
  }
